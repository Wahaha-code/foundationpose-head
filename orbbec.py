...
# Optional local SDK path. Prefer an installed pyorbbecsdk, or set PYORBBECSDK_LIB.
_sdk_lib = os.environ.get('PYORBBECSDK_LIB')
if _sdk_lib:
    sys.path.append(_sdk_lib)
else:
    _local_sdk = os.path.join(os.path.dirname(__file__), 'pyorbbecsdk', 'install', 'lib')
    if os.path.isdir(_local_sdk):
        sys.path.append(_local_sdk)
from pyorbbecsdk import VideoFrame, OBFormat, OBConvertFormat, FormatConvertFilter
