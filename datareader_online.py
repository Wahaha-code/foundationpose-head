...
class YcbineoatReader:
  def __init__(self,video_dir, downscale=1, shorter_side=None, zfar=np.inf):
    ...
    self.zfar = zfar
    self.color_files = {os.path.basename(path): path for path in sorted(glob.glob(f"{self.video_dir}/rgb/*.png"))}
    self.depth_files = {os.path.basename(path).replace('rgb', 'depth'): path
                            for path in sorted(glob.glob(f"{self.video_dir}/depth/*.png"))}
    self.K = np.loadtxt(f'{code_dir}/demo_data/demo/cam_K_ob.txt').reshape(3,3)
    self.id_strs = []
  ...
  def get_color(self, filename):
      # Directly access the dictionary for the file path
      file_path = self.color_files.get(filename)
      if file_path:
          color = imageio.imread(file_path)[...,:3]
          color = cv2.resize(color, (self.W, self.H), interpolation=cv2.INTER_NEAREST)
          return color
      else:
          logging.error(f"File {filename} not found in the color files.")
          return None
  ...
  def get_depth(self, filename):
        # Adjust the filename to match depth file convention if necessary
        depth_filename = filename.replace('rgb', 'depth')
        file_path = self.depth_files.get(depth_filename)
        if file_path:
            ...
            logging.error(f"Depth file {depth_filename} not found.")
            return None
...
class BopBaseReader:
  def __init__(self, base_dir, zfar=np.inf, resize=1):
    if base_dir is None:
      base_dir = os.path.join(code_dir, 'datasets', 'LINEMOD-O', 'lmo_test_all', 'test', '000002')
    if base_dir is None:
      base_dir = os.path.join(code_dir, 'datasets', 'LINEMOD', 'lm_test_all', 'test', '000001')
...
class LinemodOcclusionReader(BopBaseReader):
  def __init__(self,base_dir=None, zfar=np.inf):
...
class LinemodReader(LinemodOcclusionReader):
  def __init__(self, base_dir=None, zfar=np.inf, split=None):
    super().__init__(base_dir, zfar=zfar)
    self.dataset_name = 'lm'
    if split is not None:  # train/test
      with open(f'{os.getenv("LINEMOD_PREPROCESSED_DIR", os.path.join(code_dir, "datasets", "LINEMOD", "Linemod_preprocessed", "data"))}/{self.get_video_id():02d}/{split}.txt','r') as ff:
