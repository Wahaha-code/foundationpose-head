...
import yaml
import concurrent.futures  # 并行执行任务
...
class FoundationPose:
  ...
  #   set_seed(0)
  #   logging.info('[register()] Start registration optimization')

  #   if self.glctx is None:
  #       self.glctx = glctx if glctx else dr.RasterizeCudaContext()

  #   # **检查 depth 是否为 None 或格式不对**
  #   if depth is None or not isinstance(depth, np.ndarray):
  #       logging.error("[register()] ERROR: depth is None or not a numpy array")
  #       return np.eye(4)

  #   if len(depth.shape) == 3 and depth.shape[-1] == 1:
  #       depth = depth.squeeze(-1)  # 确保是 (H, W)
  ...
  #   if valid.sum() < 4:
  #       logging.info('[register()] Valid points too few, returning identity pose')
  #       pose = np.eye(4)
  ...
  #       logging.error(f"[register()] ERROR: center must be array-like, got {type(center)}")
  #       return np.eye(4)

  #   # **更新位姿中心**
  #   poses[:, :3, 3] = torch.as_tensor(np.array(center).reshape(1, 3), device='cuda')

  #   # **批量计算 ADD 误差**
  #   add_errs = self.compute_add_err_to_gt_pose(poses)
  #   min_err = add_errs.min().item()
  #   logging.info(f"[register()] After viewpoint adjustment, min ADD error: {min_err}")
  ...
  #   logging.info(f"[register()] Sorted scores: {scores}")
